...
int main()
{
	...
	Shader objectShader(getResPath("object.vsh").c_str(), getResPath("object.fsh").c_str());
	Shader lampShader(getResPath("lamp.vsh").c_str(), getResPath("lamp.fsh").c_str());

	// Set up vertex data (and buffer(s)) and attribute pointers
	//前三位是顶点坐标，后三位是法向量
	//（法向量本应该由在着色器由任意两条向量叉乘计算而得，但是这里是正方体，法向量是固定的，所以可以直接传）
	GLfloat vertices[] = {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindVertexArray(containerVAO);
		glm::mat4 model;
		model = glm::rotate(model, -55.0f, glm::vec3(1.0f, 1.0f, 0.0f));
